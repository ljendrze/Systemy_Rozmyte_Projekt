...
filename = 'obiekt_zmiana_zaklocenia.mat';
...
% wody dopływającej.
disturbanceValues = [ 5, 10, 15, 20, 25, 30, 35 ];
disturbanceTrajectories = tanks.sstate.FD * ...
   ones( simulationLength, length( disturbanceValues ) );
for i = 1 : length( disturbanceValues )
   for j = 100 / dt : simulationLength
      disturbanceTrajectories(j,i) = disturbanceValues(i);
   end
end
...
for i = 1 : length( disturbanceValues )
   fprintf('Simulation %d out of %d.', i, length( disturbanceValues ) );
   ...
   inputTrajectory = tanks.sstate.F1*ones( size( disturbanceTrajectory ) );
